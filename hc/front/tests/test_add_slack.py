...
class AddSlackTestCase(BaseTestCase):
    ...
    def test_webhook_instructions_work(self):
        ...
        response = self.client.get("/integrations/add_slack/")
    ...
    def test_slack_button(self):
        self.client.login(username="alice@example.org", password="password")
        response = self.client.get("/integrations/add_slack/")
        self.assertContains(response, "slack.com/oauth/authorize", status_code=200)

    @override_settings(SLACK_CLIENT_ID="foo")
    def test_landing_page(self):
        response = self.client.get("/integrations/add_slack/")
        self.assertContains(response, "Before adding Slack integration",
                            status_code=200)
    ...
    def test_it_handles_oauth_response(self, mock_post):
        ...
        self.assertContains(response, "The Slack integration has been added!")
